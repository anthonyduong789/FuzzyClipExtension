...
export function initSettings(state, domRefs) {
  ...
  settingBookmarkUI(state.settings.start_on_bookmarkmode, domRefs);
}
...
export function initSettingsEvents(state, domRefs) {
  ...
  domRefs.switchBookmarkSettings.addEventListener("change", (e) => {
    if (e.target.checked) {
      ...
      state.newSettings.start_on_bookmarkmode = false;
    }
    domRefs.saveSettingsButton.style.display = "block";
...
function autoScrollTick(state, domRefs) {
  ...
  if (state.drag.lastClientY < rect.top + state.drag.SCROLL_ZONE) {
    const dist =
      (rect.top + state.drag.SCROLL_ZONE - state.drag.lastClientY) /
      state.drag.SCROLL_ZONE;
    speed = -dist * state.drag.MAX_SPEED;
  } else if (state.drag.lastClientY > rect.bottom - state.drag.SCROLL_ZONE) {
    const dist =
      (state.drag.lastClientY - (rect.bottom - state.drag.SCROLL_ZONE)) /
      state.drag.SCROLL_ZONE;
    speed = dist * state.drag.MAX_SPEED;
  }

  // when cursor is above the rect.top of results frame will max
