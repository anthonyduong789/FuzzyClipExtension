...
function onTopBarMouseUp(e) {
  ...
  personal_settings.top = Number(
    String(wrapper.style.top).replace(/[^\d.-]/g, ""),
  );
  personal_settings.left = Number(
    String(wrapper.style.left).replace(/[^\d.-]/g, ""),
  );
...
function makeHandle(el, resizeW, resizeH) {
  el.addEventListener("pointerdown", (e) => {
    ...
    function onUp() {
      ...
      personal_settings.height = Number(
        String(wrapper.style.height).replace(/[^\d.-]/g, ""),
      );
      personal_settings.width = Number(
        String(wrapper.style.width).replace(/[^\d.-]/g, ""),
      );
